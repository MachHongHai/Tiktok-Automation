...
def processing_device_preference() -> str:
    if os.getenv("AUTODUB_FORCE_CPU", "").strip().lower() in {"1", "true", "yes"}:
        return "cpu"
    preference = os.getenv("AUTODUB_PROCESSING_DEVICE", "cpu").strip().lower()
...
def configure_processing_device(preference: str) -> str:
    ...
    os.environ["AUTODUB_PROCESSING_DEVICE"] = normalized
    os.environ.pop("AUTODUB_FORCE_CPU", None)
