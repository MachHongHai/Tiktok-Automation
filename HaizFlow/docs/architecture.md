...
HaizFlow is a local-first Windows application. The desktop process owns the user interface, job state, and orchestration. Media processing happens in Python pipeline modules and external command-line tools. No HTTP backend, browser client, or cloud database is part of the runtime architecture.
...
Every update persists progress, stage detail, current/total item counts, timestamps, and a `MediaSource` provenance record. Video metadata uses schema v4 in `job.json`, a per-video lock, atomic replacement (`temp -> fsync -> replace`), and the last valid `.bak` copy, so UI polling cannot observe a partially written file. Schema v2 automatically receives `media_source.type = local_file`; schema v4 binds each video to its immutable `project_id` and `project_key`; unversioned metadata migrates through every ordered step and preserves its pre-migration JSON as `.schema-migration.bak`. A schema newer than the running application is rejected rather than rewritten. The QML progress surface reads this persisted data rather than maintaining a separate progress model.
...
`core.paths` separates mutable data from source code and from the frozen executable bundle. `RUNTIME_DATA_DIR` controls the app-level cache, models, settings, diagnostics, and project index. `projects.json` is protected by an OS-level interprocess lock and atomic replacement. A valid previous index is retained as `.bak`; corrupt input is copied to a timestamped quarantine file and recovered by merging the backup with `.haizflow-project.json` manifests discovered under registered project roots. An unrecoverable parse error blocks all index writes, preventing accidental replacement with an empty list. Project records and manifests use schema v4 with ordered migration and pre-migration backups. Each project has an immutable UUID-backed `project:<uuid>` identity, so projects can share a display name without sharing inputs, exports, or deletion operations. Each project owns all of its video data and can be stored under the location selected when creating that project. New projects use an immutable `<display-name>--<project-uuid>` storage directory; the display name is never the storage identity. Legacy roots remain in place so persisted absolute paths keep working.
...
The unified dependency set contains a CUDA-capable Torch build for GPU systems and `llama-cpp-python` for CPU translation. CUDA Torch remains able to execute CPU inference on systems without an NVIDIA driver, so one environment and one installer cover both paths. The release lock pins Torch, Torchaudio, and Torchvision to their `+cu128` wheels and records hashes from the PyTorch and llama-cpp indexes. `scripts/verify-dependency-lock.py` verifies source fingerprints, exact versions, hashes, target platform, and the installed environment; `scripts/verify-runtime.py` includes that result in the build gate. `core.runtime_probe` validates Torch CPU execution, CUDA allocation, CTranslate2, WhisperX, and the selected translation backend in a disposable process before models are warmed or a device switch is committed. Native DLL, driver, AVX, and Windows commit-limit crashes are therefore reported without terminating the Qt process. `scripts/build-exe.ps1` runs this verifier before PyInstaller. `-IncludeCpuModel` embeds the prepared GGUF model; `-IncludeGpuModel` embeds the prepared Transformers snapshot for a fully offline dual-backend installer.
